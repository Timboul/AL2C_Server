...
@NamedQueries({
    ...
    , @NamedQuery(name = "Invitation.findNotInvited", query = "SELECT c FROM Contact c WHERE c.utilisateurId = :utilisateurId " +
            "AND c.id NOT IN(SELECT DISTINCT d.id from Contact d, Invitation i, Evenement e where d.id = i.invitationPK.contactId AND i.invitationPK.evenementId = :evenementId)")
})
